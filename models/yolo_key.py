        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    ...
                    # y[..., 5:5 + self.no] = y[..., 5:5 + self.no] * 4 - 2
                    ...
                    a = torch.cat([self.grid[i], self.grid[i], self.grid[i], self.grid[i]], dim=4)
                    b = torch.cat([self.anchor_grid[i], self.anchor_grid[i], self.anchor_grid[i], self.anchor_grid[i]],  dim=4)

                    xy = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]  # xy
                    wh = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh

                    lmk = (y[..., 5 + self.nc:13 + self.nc] * 4 - 2) * b + a.to(x[i].device) * self.stride[i]  # landmark x1 y1
                    y = torch.cat((xy, wh, y[..., 4:self.nc + 5], lmk), -1)
